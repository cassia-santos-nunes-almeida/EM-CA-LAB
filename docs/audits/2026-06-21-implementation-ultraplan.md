...
| 11 | **RETINT — owner chose the FULL "engaging, not AI-generic" redesign** (2026-06-22, overriding the panel's minimal-accent recommendation). A real design workstream: **visual mockups FIRST** (owner's durable preference — show visuals, not prose), then apply across gate/body/sim chrome (~30 gate sites / ~58-file occ). Interacts with #10 (both touch `SectionLayout`). **Mockups done + folded into the app-wide [Navigation & Shell Spec](../design/2026-06-23-navigation-shell-spec.md) (2026-06-23):** warm "Lab Instrument" tokens, landing Part-accordion, 3-level scroll-spy sidebar, collapse/icon-rail + auto-collapse, mobile, all 25 sections. | L (full) | **DECIDED: full redesign** | Owner decision taken; mockups approved through iteration. Next sub-gate: owner sign-off on the spec before implementation. |
