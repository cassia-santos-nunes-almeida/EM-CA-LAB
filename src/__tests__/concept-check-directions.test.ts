 ...
 * audit's directional CCs (§A.4): lenz (clockwise), lorentz (+y), and polarization
 * handedness (Q_JONES, RCP = the −i Jones vector).
 *
 * The other §A.4 candidates were assessed and DO NOT belong here — the invariant is
 * only defined when ≥2 options can name the same direction:
 *   • faraday — its CCs key magnitudes / law names (e.g. "50 V", "Lenz's law"), no
 *     option names a rotation/sign direction; the motional-EMF CC asks a magnitude.
 *   • ampere Q_RHR — the answer is a physical QUANTITY ("conventional current"); the
 *     options are field/current/force, not competing left/right directions.
 */
